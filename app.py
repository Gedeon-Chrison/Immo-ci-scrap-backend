...
def init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS annonces (
            ...
            transaction TEXT, type_bien TEXT,
            zone TEXT, quartier TEXT,
            prix TEXT, prix_fcfa INTEGER DEFAULT 0,
            surface TEXT, pieces TEXT,
            contact TEXT, agence TEXT,
            description TEXT, url TEXT,
            photos TEXT DEFAULT '[]',
            publie_at TEXT, scraped_at TEXT,
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS scrape_jobs (
            ...
            status TEXT, total INTEGER DEFAULT 0, config TEXT
        )
    """)
    conn.commit()
    conn.close()
...
def get_annonces():
    ...
    if tx: q += " AND transaction = ?"; params.append(tx)
    if zone: q += " AND zone LIKE ?"; params.append(f"%{zone}%")
    if type_bien: q += " AND type_bien LIKE ?"; params.append(f"%{type_bien}%")
    if min_prix: q += " AND prix_fcfa >= ?"; params.append(int(min_prix))
    if search:
        q += " AND (zone LIKE ? OR quartier LIKE ? OR description LIKE ? OR type_bien LIKE ?)"
        s = f"%{search}%"
        params.extend([s, s, s, s])
    if sort == "price_asc": q += " ORDER BY prix_fcfa ASC"
    elif sort == "price_desc": q += " ORDER BY prix_fcfa DESC"
    elif sort == "surface": q += " ORDER BY CAST(surface AS INTEGER) DESC"
    else: q += " ORDER BY scraped_at DESC"
    q += f" LIMIT {limit}"
    ...
    for r in rows:
        a = dict(r)
        try: a["photos"] = json.loads(a.get("photos") or "[]")
        except: a["photos"] = []
        annonces.append(a)
...
def get_stats():
    ...
    conn.close()
    return jsonify({
        "total": total, "ventes": ventes,
        "locations": locations, "new_24h": new_24h,
...
def launch_scrape():
    ...
    config = {
        "zones": body.get("zones", ["Cocody","Riviera","Riviera Golf","Marcory","Zone 4","Biétry","2 Plateaux","Angré"]),
        "types": body.get("types", []),
        "min_loyer": int(body.get("min_loyer", 1_000_000)),
        "min_vente": int(body.get("min_vente", 100_000_000)),
        "days": int(body.get("days", 7)),
        "sources": body.get("sources", []),
    }
    thread = threading.Thread(target=run_scrape, args=(DB_PATH, config), daemon=True)
    thread.start()
    return jsonify({"message": "Scraping lancé", "config": config})
